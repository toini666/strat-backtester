"""Replay the winner multi-asset preset and verify it reproduces the
expected metrics. Also runs through the actual backend `run_multi_backtest`
code path to confirm our harness `_combined_metrics` is consistent with what
the UI Dashboard / FavoritesPage would show.
"""
...
def main():
    ...
    # Cross-check: run through backend's actual multi-asset code path
    print()
    print("Cross-check: backend run_multi_backtest (UI-equivalent path)")
    ...
    # Re-compute DD$ from backend's combined trades
