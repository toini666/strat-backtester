"""Campaign-local constants and helpers for HMASSLOsciV3 MNQ+MGC multi-asset."""
...
# Baseline params from the "HMA-SSL-V3 - MNQ/MGC - Best" preset (preset id 05f42cde-…).
...
# Baseline risk per the saved preset.
MNQ_BASE_RISK = 0.0036
MGC_BASE_RISK = 0.0047
...
def base_engine_mnq() -> BacktestEngineSettings:
    """Reproduce baseline blackouts for MNQ leg: 22-23:59, 11-12, 14-15 active."""
...
def base_engine_mgc() -> BacktestEngineSettings:
    """Reproduce baseline blackouts for MGC leg: 22-23:59, 11-12, 6-7, 7-8, 3-4, 9-10 active."""
    return BacktestEngineSettings(
        auto_close_enabled=True, auto_close_hour=22, auto_close_minute=0,
        blackout_windows=[
            _bw(False, 0, 0, 0, 5),
...
def _trades_with_source(trades: List[Dict[str, Any]], src: str) -> List[Dict[str, Any]]:
    out = []
    for t in trades:
        # Convert Trade pydantic objects (or plain dicts) to dicts we can mutate.
...
def _combined_dd_dollars(initial_equity: float, trades: List[Dict[str, Any]]) -> Tuple[float, float, float]:
    """Walk merged trades sorted by entry_time and return (net_pnl, max_dd_dollars, max_dd_pct)."""
...
def run_multi(
    *,
    ...
    """Run MNQ + MGC legs and combine per backend multi_asset logic.

    Returns dict with: net_pnl, max_dd_$, max_dd_%, trades_total, wr,
    profit_factor, mnq_pnl, mgc_pnl, leg1_summary, leg2_summary,
    (optionally) trades.
    """
    ...
    # Combined daily limits are taken from MNQ engine (cfg1) per backend convention.
