"""Phase 5 — Indicator length sweep on Phase 4 winner.

Phase 4 winner: min_gap=8 + sl_lookback=15 + rr_tp=3.0 + sl_max=50 + ut_on=False
+ sig_extreme_filter_on=True + hw_extreme=15  → PnL=$42,778, DD=$3,044, P/DD=14.05.

Per memory note (feedback_parameter_sweeps): mf_length is known non-monotone on
HMASSLOsciV3, sweep finely. ut_* params now irrelevant (ut_on=False).
"""
...
SWEEPS = {
    # Oscillator (mf_length non-monotone; finer grid)
...
def main() -> int:
    ...
    print()
    print("=" * 100)
    print("Top 15 individual sims by P/DD")
    print("=" * 100)
