...
def _install_fake_indicators(
    ...
    *,
    ...
    hma1 = pd.Series(np.full(length, 110.0), index=index)
    hma2 = pd.Series(np.full(length, 100.0), index=index)

    flip_idx = signal_idx - 6
    hma1.iloc[:flip_idx] = 94.0
    hma2.iloc[:flip_idx] = 96.0
    hma1.iloc[flip_idx] = 96.0
    hma2.iloc[flip_idx] = 94.0
...
def test_hma_ssl_osci_v2_removes_hma_vs_ssl_gate(monkeypatch):
    data = _build_data()
    params = _params()
    signal_idx = 30
...
def test_hma_ssl_osci_v2_uses_last_hma_cross_level_for_stop(monkeypatch):
    data = _build_data()
    params = _params()
    signal_idx = 30
    ...
    assert v1["long_entries"].iloc[signal_idx]
    assert v2["long_entries"].iloc[signal_idx]
    assert v1["sl_long"].iloc[signal_idx] == 98.0
    assert v2["sl_long"].iloc[signal_idx] == 95.0
